...
class DailyRatings(models.Model):
    science_group = models.ForeignKey(ScienceGroup, on_delete=models.CASCADE,
                                      related_name='dailyratings_to_science_group')
    student = models.ForeignKey('account.CustomUser', on_delete=models.SET_NULL, null=True,
                                related_name='dailyratings_to_student')
    ball = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    def __str__(self):
        return f"DailyRatings #{self.id}"

class NBandRating(models.Model):
    lesson = models.ForeignKey(Lesson, on_delete=models.SET_NULL, null=True, blank=True, related_name='nbandrating_to_lesson')
